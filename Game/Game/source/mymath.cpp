...
// 円同士の当たり判定
bool IsHitCircle(Character* c1, Character* c2)
{
	// 「2つの円の中心点」の距離を求める
	// ピタゴラスの定理（三平方の定理）で求めることができる
	// x,zで処理。yは判定せず
	float w, h, length;
	w = c1->GetPosition().x - c2->GetPosition().x;
	h = c1->GetPosition().z - c2->GetPosition().z;
	length = (int)sqrt((float)(w * w + h * h));

	// 中心点間の距離が、2つの円の半径の合計よりも小さい場合、当たり
	if (length < c1->GetColR() + c2->GetColR())
	{
		return true;
	}
...
// カプセルと球の当たり判定
bool IsHitCapsuleSphere(const VECTOR& capsuleTop, const VECTOR& capsuleBottom, float capsuleRadius, const VECTOR& sphereCenter, float sphereRadius)
{
